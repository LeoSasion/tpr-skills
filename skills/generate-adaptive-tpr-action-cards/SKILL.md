...
description: Generate, revise, resume, quality-check, package, deliver, or convert bilingual TPR action cards for any user-provided character reference, including people, animals, toys, mascots, robots, and stylized figures. Use for photo-only clickable setup, a mandatory two-round model-curated wardrobe choice with per-card single-range color/style assignments and child/adult masculine/feminine or shared scopes, explicit default-four-worker-or-custom subagent concurrency with an orchestration-only primary agent, optional user-specified backgrounds, explicit generation model/API/Skill selection, maximum within-range outfit diversity, the built-in 200-action library, a four-sample approval gate, character-consistent image editing, safe identifiers, action semantics, stateful retries, A4 composition, exact bilingual captions, visual QA, ZIP delivery, Word output, or `/img2word` and “做成word” requests.
---
...
Read [references/interaction-flow.md](references/interaction-flow.md), [references/wardrobe-choice.md](references/wardrobe-choice.md), and [references/wardrobe-option-library.md](references/wardrobe-option-library.md) before asking any setup or confirmation question. Detect whether the current host exposes a native input control before emitting a widget. Never assume a literal `genui…` marker will render; when no native control is available, omit the marker entirely and show the complete visible numbered fallback. When the user directly uploads one or more photos for a new case without execution settings beyond a generic request to make cards, first show the two clickable entry choices defined there: automatic route or guided setup. Both routes must then pass through the mandatory two-round wardrobe choice, a concurrency-only screen, and a generation model/interface screen before planning or generation. Do not ask a routine background question. When the user has not explicitly specified a background, record `background_mode=unspecified`, leave `background_treatment` empty, and omit every background label, placeholder, and instruction from the image prompt. Prefer native clickable option cards for every finite question throughout the workflow; require typed input only for a custom concurrency integer, custom interface details, or an irreducible ambiguity.

The automatic route has four gates: complete the sequential color-direction and style-family choices; answer the concurrency-only screen; choose and capability-check the generation model/interface; then generate and QA four representative samples and wait for confirmation before expanding to all 200 actions. The model must curate each wardrobe set from visible evidence and the approved library; never shuffle or sample library rows mechanically. Reuse the four passed samples in the full batch.
...
- Wardrobe library version, selection schema version, recommendation method and fingerprint, non-sensitive evidence basis, canonical selected color/style range sets with round/option provenance, and any exact custom override.
- For a v2 manifest, stable `wardrobe_selection_schema_version=2`, canonical `wardrobe_selected_ranges_json`, and `wardrobe_assignment_strategy=balanced-scattered-v1`; per-row `assigned_color_direction_key` and `assigned_style_family_key`; and profile-v2 `wardrobe_range_pools_json` keyed by the selected ranges.
...
- Background state: default new work to `background_mode=unspecified` with an empty `background_treatment`. Use `pure-white` only after an explicit pure-white request, or `specified` with the user's exact non-empty treatment after another explicit background request. Preserve `auto-varied` only as legacy/resume evidence.
...
Resolve wardrobe direction before final batch planning. Unless the user already confirmed at least one broad color range and at least one broad style range in the current turn (or the profile is `fixed`/`none`), force the two sequential four-option screens in [references/wardrobe-choice.md](references/wardrobe-choice.md). These are the only routine screens that allow a multi-select union; concurrency, model/interface, delivery, and other mutually exclusive questions remain single-select. In Round 1, the model recommends three materially different broad color directions from the library using stable visible appearance, neutral proportions, apparent-life-stage safety, anatomy, user preference, and print readability. Before Round 2, resolve an internal clothing-presentation scope from the approved profile: child/adult × masculine/feminine only for clear medium/high-confidence human or explicitly `human-biped` stylized presentation, otherwise `shared`. This is not a gender-identity claim or an extra routine question. A narrow scope uses its rows plus shared rows and must contribute at least two of the three visible options; a shared scope uses shared rows only. Round 2 conditions eligible styles on the complete selected color set. Accept any non-empty subset of options 1–3, including `1+2`, and accept a current visible range name directly. Option 4 `更多其他` is a loop-control value: when combined with selections, retain and deduplicate those selections, refresh with next-best unshown IDs, and never place 4 in the selected set or fingerprint. Direct-name matching first uses current visible labels, then the applicable age domain and style group; ambiguous matches require clarification. An unknown adult name may use the exact custom route, while child or age-uncertain input may resolve only to an eligible child-safe library row. Never use RNG, a seed, shuffled rows, or the profile's Cartesian factor pool to choose visible recommendations. After both rounds, use `specified` mode, fingerprint the canonical selected sets, build profile-v2 range-keyed pools, and keep the selected-set contract stable across the batch. Assign each row exactly one selected color key and one selected style key using `balanced-scattered-v1`; never blend two ranges from the same dimension in one image. Ask only when multiple uploaded characters, visual uncertainty, duplicate labels, or an irreducible custom requirement needs a decision; present finite resolutions as clickable choices.

After the wardrobe rounds, force one concurrency-only screen unless that answer is already explicit or recorded. Treat `subagent_parallelism=enabled` as explicit permission for four simultaneous image-generation child workers while the primary agent remains orchestration-only; treat `disabled` as serial generation by the primary agent; after `custom`, ask for and record the exact number of image-generation child workers. The primary agent does not count toward `subagent_concurrency` in either parallel mode. Validate that the host has the primary slot plus the requested child-worker slots and that the selected image interface can sustain the requested image throughput before final planning. Never silently reduce it or use the primary agent to fill a missing worker slot. Do not elicit or invent a background. If the user gave no background requirement, use `unspecified` plus an empty treatment; if the user explicitly requested pure white, use `pure-white`; for another explicit requirement, use `specified` and record it verbatim.
...
If the photo-only entry uses the automatic route, complete the wardrobe, separate concurrency, and generation route choices first; only then set `adaptation_mode=specified`, set delivery to `both`, and set Word identifier visibility to `hidden`. Otherwise, if the current request does not explicitly state an output format, ask with clickable choices. Do not infer ZIP, concurrency, or generation route from the automatic label. Default only the absent background to `unspecified`; never turn absence into a generated scene description. Treat `/img2word` or “做成word” as an explicit Word choice and default its identifier visibility to `hidden` unless the user asks to show numbers. Record both wardrobe rounds, the final selected-set fingerprint, v2 selected-range and per-row assignment fields, profile-v2 range pools, concurrency, background state, generation route, and all other decisions before generation or packaging. Treat an absent or `1` wardrobe selection schema as legacy single-select verify-only evidence: preserve and verify passed work, but never use it to drive a new recommendation, assignment, or finalization.
...
- [references/wardrobe-choice.md](references/wardrobe-choice.md) before presenting wardrobe options or recording selected color/style range sets.
...
- Use a full-body composition. Under `unspecified`, make no background request and judge no planned background treatment; the model is free to choose it. Under `pure-white` or `specified`, enforce only the user's explicit background requirement. Apply the profile's wardrobe policy and the row's one `assigned_color_direction_key` plus one `assigned_style_family_key`; never combine multiple color ranges or multiple style ranges in one image. Include only an action-critical prop or partial partner cue.
...
Set `word_identifier_visibility` to `hidden` by default or `shown` only after an explicit user choice. When resuming a legacy manifest, migrate it into the current template. Use `hidden` unless the user previously chose visible Word numbers. Recover wardrobe provenance, concurrency, explicit background choices, and generation route only from evidence; when no background choice exists, record `unspecified` rather than asking or inventing one. Preserve old passed cards and package hashes while migrating fields.
...
Treat the three returned entries as recommendations only. After the non-empty color-key set is finalized, run the separate style stage conditioned on the complete color set and bound to the profile-derived style scope. After both range sets are finalized, expand every selected color key into at least four approved sub-palettes and every applicable selected style key into at least four silhouettes and four substyles; preserve the key-to-pool mapping, increment the profile version, and revalidate it. Then use `character_profile.py suggest --mode specified` to build a maximally diverse, deterministic factor sequence from the per-row assignments. Do not change identity anchors.
...
Fill the character ID, profile version and SHA-256, final adaptation mode, complete two-round wardrobe provenance, stable v2 selected-range JSON and assignment strategy, per-row assigned color/style keys, persona, wardrobe policy, concurrency policy/count, background mode and treatment when applicable, generation backend mode/interface/model, required render capabilities, action risk tags, suitability handling, adaptation status and reason, motion chain, key joints, weight shift, head angle, gaze, expression, necessary prop, outfit, color, silhouette, and substyle. Keep one persona, profile version, profile hash, final mode, canonical selected-range set, recommendation fingerprint, assignment strategy, concurrency policy/count, background mode, and generation route across a batch; allow only the two per-row assigned keys to vary inside their recorded sets. `balanced-scattered-v1` must enumerate the color × style key pairs, balance pair usage with a difference of at most one when feasible, deterministically scatter the order from the stable batch inputs, and reproduce the same assignments from the same inputs. Never group one selected range into a contiguous block or emit a mechanical `1,2,1,2...` alternation when more varied stable ordering is feasible. Each row uses factors only from its two assigned key pools in `wardrobe_range_pools_json`. A custom union such as `礼服+泳装` must become separate custom style ranges and separate keys; each row receives only one of them, and neither the joined label nor multiple custom styles may enter one row's outfit or prompt. For a suitability rule with `safety_scope=all`, copy every listed tag and exact `default_handling` and use `fallback` / `safe-override`. For `minor-only`, do that only for a child or age-uncertain profile; a clearly adult profile must instead use `action_risk_tags=none`, `suitability_handling=none`, `adaptation_status=pass`, and `adaptation_reason=ok`. Use `none` for an unlisted action. Never treat a structurally valid CSV as proof that a relationship- or consent-sensitive image is valid: apply every applicable `all`-scope `adaptation_note` and verify it visually. Across the selected range set, cover approved keys and their colors, silhouettes, and substyles before reuse; for four samples maximize distinct assigned pairs, colors, silhouettes, and substyles when feasible. Vary layering, material, texture, trim, and restrained accessories visibly, without drifting outside the row's assigned ranges or changing identity anchors.
...
Use the exact selected generation route and edit from the one selected original. For the recommended route, use the `imagegen` Skill only after the host confirms the requested `Codex 5.6 Luna Max` routing preset and callable image interface. For a custom route, read and follow the selected image-generation Skill or API contract before invocation. Send the same immutable identity, action, wardrobe, optional explicit background, and age-gated child-safety constraints regardless of backend. Do not silently substitute a model, endpoint, or Skill; block with `GEN_BACKEND_UNAVAILABLE` when capability or authentication cannot be confirmed, and record `GEN_BACKEND_POLICY` when the selected backend itself refuses an otherwise valid request. Do not translate any adult request into conservative full coverage. Build every new work packet from the current profile and finalized wardrobe selection with `scripts/build_work_packets.py`; never hand-author or copy a prior prompt. The builder emits `wardrobe_safety_scope=adult-none` as structured metadata for a clearly adult profile but emits no adult no-restriction prose and no child safety-note/avoid-feature text in its prompt; it emits the `minor-nonsexualized` prompt rule only for a child or age-uncertain profile. Under `background_mode=unspecified`, it emits no background label, heading, placeholder, or sentence. A worker must reject a missing, edited, or non-builder packet. Generate each row separately with the packet's exact prompt:
...
```text
Use the reference only to preserve the same primary character. Keep [IDENTITY ANCHORS], [APPEARANCE], [PROPORTIONS], and [APPARENT LIFE-STAGE PRESENTATION] consistent. Create a natural full-body moment of the character [ACTION AND MOTION CHAIN]. Use anatomy and balance appropriate to [CHARACTER KIND]. [HEAD ANGLE], [GAZE], [ACTION-ALIGNED EXPRESSION]. Art direction: [PERSONA]. Apply the current row's one [OUTFIT OR COSTUME TREATMENT] under the [WARDROBE POLICY], preserving [SIGNATURE FEATURES]. Add [MINOR NONSEXUALIZATION RULE] only for a child or age-uncertain profile. Add a background sentence only when the user explicitly requested one; otherwise omit the entire background concept. Action clarity: [CURRENT ROW'S MOTION AND ANATOMY CUES]. Portrait composition with generous clear space and a protected bottom caption area. No text, border, branding, unrelated props, or full additional characters. Include only [ACTION-CRITICAL PROP OR PARTIAL INTERACTION CUE] when necessary. Make [DISAMBIGUATION CUE] clearly visible and avoid [CONFUSABLE ACTION CUE].
```
...
Treat later feedback as a rule update. Preserve successful defaults, character profiles, manifests, hashes, reference mappings, wardrobe selected sets, per-row assignments, assignment strategy, concurrency policy/count, explicit background choices, generation route, Word visibility, and user-specified identity anchors. Do not silently alter passed cards. Use failure history to improve weak character-action combinations and the approved action-semantics library.
