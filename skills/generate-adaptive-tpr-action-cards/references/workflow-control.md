...
For every new `varied` or `signature-variants` batch without two confirmed wardrobe ranges, run the mandatory two-round flow from [wardrobe-choice.md](wardrobe-choice.md) before creating the action plan. Round 1 contains three model-curated broad color directions plus `更多其他`; Round 2 contains three model-curated broad style families conditioned on the selected color plus `更多其他`. Rank library IDs from non-sensitive visible evidence and card readability; never shuffle or mechanically sample options. `更多其他` refreshes only the current stage and excludes shown IDs. After both selections, set `adaptation_mode=specified`, record all provenance fields and the final fingerprint, rebuild the profile pools inside the selected ranges, increment the profile version, and revalidate. Then resolve execution/background and the separate generation route choice. Keep the recommended persona stable unless the user separately specifies one. Generate and QA four representative samples without an intermediate plan-confirmation question. Stop at the clickable sample gate. Expand to all 200 only after `样图正确，生成全套（推荐）`; reuse the four passed samples.
...
- complete two-round wardrobe provenance, consistent across all selected rows;
...
The `generated` transition rechecks that character adaptation, execution/background values, generation backend values, and plan validation passed. A row marked `blocked`, `pending`, or plan `fail` must not advance even if a raw file happens to exist.
