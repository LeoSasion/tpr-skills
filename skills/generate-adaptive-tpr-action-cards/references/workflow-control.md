...
After the two wardrobe rounds, resolve the batch-wide execution choices before planning in two consecutive, independent screens. First ask `subagent_parallelism=enabled|disabled|custom` and, when needed, the positive-integer `subagent_concurrency`; then ask `background_mode=auto-varied|pure-white`. Never place concurrency and background in the same widget or fallback message, and skip each screen independently only when its answer is already explicit or recorded. The automatic route does not silently choose either answer. `enabled` means four simultaneous image-generation child workers in addition to an orchestration-only primary agent and records `subagent_concurrency=4`; `disabled` records `1`, prohibits spawning subagents, and lets the primary generate serially; `custom` requires an exact child-worker count. Record one exact `background_treatment` per row; use `pure-white` for white mode and a concrete clean scene or studio treatment for auto-varied mode.
...
For every new `varied` or `signature-variants` batch without two confirmed wardrobe ranges, run the mandatory two-round flow from [wardrobe-choice.md](wardrobe-choice.md) before creating the action plan. Round 1 contains three model-curated broad color directions plus `更多其他`. Before Round 2, resolve the approved profile to child/adult × masculine/feminine when clear or `shared` when neutral, uncertain, low-confidence, nonhuman, or not applicable. The scope describes clothing presentation, not gender identity. Round 2 contains three eligible broad style families conditioned on the selected color plus `更多其他`; a narrow scope must contribute at least two group-specific options, while a shared target uses shared rows only. Rank library IDs from non-sensitive visible evidence and card readability; never shuffle or mechanically sample options. `更多其他` refreshes only the current stage, preserves the style scope, and excludes shown IDs. After both selections, set `adaptation_mode=specified`, record all provenance fields and the final fingerprint, rebuild the profile pools inside the selected ranges, increment the profile version, and revalidate. Then resolve the concurrency screen, the background screen, and the generation route screen in that order. Keep the recommended persona stable unless the user separately specifies one. Generate and QA four representative samples without an intermediate plan-confirmation question. Stop at the clickable sample gate. Expand to all 200 only after `样图正确，生成全套（推荐）`; reuse the four passed samples.
...
- complete two-round wardrobe provenance, consistent across all selected rows, with the selected model-curated style eligible for the profile-derived recommendation scope and active library version;
- one stable concurrency policy/count, background mode, and generation backend mode/interface/model across the batch;
- `pure-white` treatment on every white-mode row, or a recorded non-white treatment with four distinct treatments per feasible four-card window under `auto-varied`;
- every concrete wardrobe factor inside the post-selection profile pools, including `specified` mode;
- maximum feasible coverage of approved sub-palettes, silhouettes, and substyles before reuse, with balanced long-batch cycles and explicit restrictions honored;
...
The `generated` transition rechecks that character adaptation, the separately confirmed concurrency and background values, generation backend values, and plan validation passed. A row marked `blocked`, `pending`, or plan `fail` must not advance even if a raw file happens to exist.
